...
STYLE_IMAGE = 'images/caricature.png'
CONTENT_IMAGE = 'images/content.jpg'
CARI_IMAGE = 'images/caricature.png'
...
def content_loss_func(sess, model):
    ...
    def _content_loss(p, x):
        ...
        return (1.0 / 2.0) * tf.reduce_sum(tf.pow(x - p, 2))
...
def style_loss_func(sess, model):
    ...
    def _style_loss(a, x):
        ...
        result = (1.0 / (4.0 * N**2 * M**2)) * tf.reduce_sum(tf.pow(G - A, 2))
        return result
...
def run():
    ...
    cari_image = load_image(CARI_IMAGE)
    ...
    sess.run(model['input'].assign(cari_image))
    cari_loss = content_loss_func(sess, model)

    
    total_loss = ALPHA * content_loss * 0.5 + ALPHA * cari_loss * 0.5 + BETA * style_loss
    ...
    now = datetime.now()
    case = "result/{:.4f}".format(ALPHA/BETA)+"_"+str(ITERATIONS)+"_"+now.strftime("%Y%m%d_%H%M%S")
    ...
    for it in range(ITERATIONS+1):
        ...
        if it%save_it == 0:
            ...
            filename = case+'/output/%05d.png' % (it)
